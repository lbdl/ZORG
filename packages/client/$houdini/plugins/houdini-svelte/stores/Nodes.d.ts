import type { Nodes$input, Nodes$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class NodesStore extends QueryStore<Nodes$result, Nodes$input> {
	/**
	 * ### Route Loads
	 ...
	 * 	const store = new NodesStoreStore()
 ...
 *
 * ```js
 * import { load_Nodes } from '$houdini';
 ...
 *   };
 *
 *   return await load_Nodes({ event, variables });
 * };
 * ```
 *
 ...
 *
 * ```js
 * import { loadAll, load_Nodes } from '$houdini';
 * import type { PageLoad } from './$types';
 *
 * export const load: PageLoad = async (event) => {
 *   const variables = {
 ...
 *     load_Nodes({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_Nodes: (params: QueryStoreFetchParams<Nodes$result, Nodes$input>) => Promise<{Nodes: NodesStore}>
